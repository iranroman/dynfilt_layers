...
class Conv2D(tf.keras.layers.Layer):
  ...
  def call(self, X, kernel):
    ...
    out = tf.nn.depthwise_conv2d(
              ...
              filter=K,
              strides=[1, 1, 1, 1],
              padding=self.padding) # here no requirement about padding being 'VALID', use whatever you want.
    ...
    if self.padding == "VALID":
        out = tf.reshape(out, [H-k_height+1, W-k_width+1, batch_size, nchans_in, nchans_out])
    ...
    #######################################################
    out = tf.transpose(out, [2, 0, 1, 3, 4]) # shape (batch_size, H, W, nchans_in, nchans_out)
